...
import { IconCloud, IconSearch, IconUser, IconChevronRight, IconAlertCircle, IconRefresh } from '@tabler/icons-react';
...
function UserAvatar({ src, name }: { src?: string | null; name: string }) {
  ...
  if (!src)
    return (
      <div className="w-11 h-11 rounded-full bg-muted border border-border flex items-center justify-center shrink-0 text-muted-foreground font-semibold text-sm">
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          >
            <IconCloud size={20} style={{ color: '#ED4C27' }} />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-foreground">Cloud</h1>
            <p className="text-sm text-muted-foreground">Assets verwalten</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 text-sm bg-muted border border-border rounded-lg text-foreground placeholder-gray-500 focus:outline-none focus:ring-2 focus:border-transparent transition-all"
      ...
      <Card className="rounded-2xl overflow-hidden divide-y divide-border">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-16 text-center px-4">
            <IconCloud size={36} className="text-foreground" />
          ...
          filtered.map((user) => (
            <div key={user.fourbased_id} className="flex items-center gap-4 px-5 py-4 hover:bg-accent/50 transition-colors">
